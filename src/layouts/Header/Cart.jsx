...
const CartIcon = () => {
    ...
    useEffect(() => {
        if (JSON.stringify(prevItems.current) !== JSON.stringify(cartItems)) {
            ...
            const fetchCourseDetails = async () => {
                try {
                    const details = await Promise.all(cartItems.map(course =>
    ...
    return (
        ...
        >
            ...
            {isHovered && (
                <div className="cart-menu" style={{ minHeight: '50px', maxHeight: '400px', display: 'flex', flexDirection: 'column' }}>
                    {accessToken ? (
                        <>
                            <div className="cart-items" style={{ flex: '1 1 80%', overflowY: 'auto' }}>
                                {cartItems?.length > 0 ? (
