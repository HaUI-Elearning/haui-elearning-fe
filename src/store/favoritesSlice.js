...
export const fetchFavoriteItems = createAsyncThunk(
    ...
    async (accessToken) => {
        try {
            const response = await axios.get('http://localhost:8080/api/v1/favorite-courses', {
                headers: {
...
export const addToFavoritesApi = createAsyncThunk(
    ...
    async ({ courseId, accessToken }, { dispatch }) => {
        try {
            await axios.post(`http://localhost:8080/api/v1/favorite-courses?courseId=${courseId}`, {}, {
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                },
            });
            
            return dispatch(fetchFavoriteItems(accessToken));
        } catch (error) {
...
export const removeFromFavoritesApi = createAsyncThunk(
    ...
    async ({ courseId, accessToken }) => {
        try {
            const response = await axios.delete(`http://localhost:8080/api/v1/favorite-courses?courseId=${courseId}`, {
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                },
            });
